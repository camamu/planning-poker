...
| Persistencia              | **Postgres gestionado en Supabase** (free tier), acceso con `pg` + **Kysely**                  | _Revisado — ver nota abajo._ Kysely sigue siendo un query builder tipado, no un ORM: los mappers persistencia↔dominio quedan explícitos, que es justo lo que la hexagonal necesita. Solo cambia `DATABASE_URL`; el dominio no se entera de dónde vive Postgres |
| Migraciones               | Kysely migrations, versionadas en `apps/api/migrations`, aplicadas contra Supabase             | —                                                                                                                                                                                                                                                              |
...
| Despliegue                | **API en Render (free) · frontend en Cloudflare Pages (free) · BD en Supabase (free)**         | _Revisado — ver nota abajo._ Sin Redis en v1: una sola instancia, el broadcast en memoria basta. El puerto `RealtimeBroadcaster` queda por si algún día hace falta más de una                                                                                  |

**Nota sobre el giro de Supabase y el hosting (revisión posterior al cierre inicial):** la decisión original descartaba Supabase por encaje, no por calidad, y planteaba un VPS propio con Dokploy. Se revisó al priorizar maximizar las opciones de despliegue gratuito: con la base de datos fuera del contenedor, la API queda como un único proceso ligero, lo que abre plataformas gratuitas de un solo servicio (Render, Koyeb) que no encajaban bien con Postgres al lado. El detalle completo, incluidos los dos avisos operativos que trae este cambio (pausa de Supabase a los 7 días, sueño de Render por inactividad), está en `06-despliegue.md`. La arquitectura hexagonal es precisamente lo que hace este giro barato: nada fuera de `infrastructure/persistence` se entera.
...
| 9   | Producción                    | Dockerfile multi-stage, healthchecks, los cinco workflows de CI/CD y el versionado automático                                                                                                     |
| 10  | Despliegue gestionado         | API en Render, front en Cloudflare Pages, BD en Supabase: `deploy.yml` con migraciones, `heartbeat.yml` y la documentación vigente (ver `06-despliegue.md`)                                       |
